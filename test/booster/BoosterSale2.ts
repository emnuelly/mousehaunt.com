...
describe("Booster Sale 2", function () {
  let deployer: SignerWithAddress; // only here for clarity
  ...
  before(async function () {
    [deployer, boosterOwner, buyer, buyer2, buyer3] = await ethers.getSigners();
  });
  ...
  beforeEach(async function () {
    ...
    boosterSale = await BoosterSale.deploy(boosterOwner.address, busd.address, bmhtl.address, bmhte.address, legendaryPrice, epicPrice);
  ...
  describe("Whitelist", async function() {
    it("should set the whitelist correctly", async function() {
      await boosterSale.connect(boosterOwner).setWhitelist(
        [buyer.address, buyer2.address],
        [2, 3],
        [5, 0]
      );

      expect(await boosterSale.whitelist(buyer.address, bmhtl.address)).to.equal(2);
      expect(await boosterSale.whitelist(buyer.address, bmhte.address)).to.equal(5);

      expect(await boosterSale.whitelist(buyer2.address, bmhtl.address)).to.equal(3);
      expect(await boosterSale.whitelist(buyer2.address, bmhte.address)).to.equal(0);
    });
    ...
    it("should allow the owner to reset the whitelist", async function() {
      ...
      expect(await boosterSale.whitelist(buyer.address, bmhtl.address)).to.equal(0);
      expect(await boosterSale.whitelist(buyer.address, bmhte.address)).to.equal(0);

      expect(await boosterSale.whitelist(buyer2.address, bmhtl.address)).to.equal(10);
      expect(await boosterSale.whitelist(buyer2.address, bmhte.address)).to.equal(10);
    });
  });
  ...
  describe("Sale", async function() {
    beforeEach(async function () {
      ...
      // users 1 and 2 approve boosterSale:
      const buyer1DesiredAllowance = legendaryPrice.mul(buyer1LegendaryAllowance).add(epicPrice.mul(buyer1EpicAllowance));
      const buyer2DesiredAllowance = legendaryPrice.mul(buyer2LegendaryAllowance).add(epicPrice.mul(buyer2EpicAllowance));
      await busd.connect(buyer).approve(boosterSale.address, buyer1DesiredAllowance);
      await busd.connect(buyer2).approve(boosterSale.address, buyer2DesiredAllowance);

      // BoosterOwner approve the max allowance
      await bmhtl
      .connect(boosterOwner)
      .approve(boosterSale.address, await bmhtl.balanceOf(boosterOwner.address));

      await bmhte
      .connect(boosterOwner)
      .approve(boosterSale.address, await bmhte.balanceOf(boosterOwner.address));
    });
    ...
    it("should allow a whitelisted user to buy tokens", async function () {
      ...
      const maxLegendaries = await boosterSale.whitelist(buyer.address, bmhtl.address);
      await boosterSale.connect(buyer).buy(bmhtl.address, toWei(maxLegendaries.toString()));

      // get whitelist allowance and buy as much Epics as I can
      const maxEpics = await boosterSale.whitelist(buyer.address, bmhte.address);
      await boosterSale.connect(buyer).buy(bmhte.address, toWei(maxEpics.toString()));
      ...
      expect(legendaryBalanceAfter.toString()).to.equal(legendaryBalanceBefore.sub(toWei(maxLegendaries.toString())));
      expect(epicBalanceAfter.toString()).to.equal(epicBalanceBefore.sub(toWei(maxEpics.toString())));
    ...
    it("should allow a partially whitelisted user to buy only the tokens they are whitelisted for", async function () {
      ...
      // get whitelist allowance and buy as much Legendaries as I can
      const maxLegendaries = await boosterSale.whitelist(buyer2.address, bmhtl.address);
      await boosterSale.connect(buyer2).buy(bmhtl.address, toWei(maxLegendaries.toString()));

      // try to buy Epics and fail because there is no whitelist allowance
      await expect(boosterSale.connect(buyer2).buy(bmhte.address, toWei("1"))).to.be.revertedWith('BoosterSale: above cap');
      ...
      expect(await boosterSale.whitelist(buyer2.address, bmhtl.address)).to.equal(0);
      expect(await boosterSale.whitelist(buyer2.address, bmhte.address)).to.equal(0);
    });

    it("should revert when a non-whitelisted user tries to buy tokens", async function () {
      // make sure the whitelist allowance is zero
      expect(await boosterSale.whitelist(buyer3.address, bmhtl.address)).to.equal(0);
      expect(await boosterSale.whitelist(buyer3.address, bmhte.address)).to.equal(0);
      ...
      await expect(boosterSale.connect(buyer3).buy(bmhtl.address, toWei("1"))).to.be.revertedWith('BoosterSale: above cap');

      // try to buy Epics and fail because there is no whitelist allowance
      await expect(boosterSale.connect(buyer3).buy(bmhte.address, toWei("1"))).to.be.revertedWith('BoosterSale: above cap');
