...
describe("Mouse Haunt Booster LEGENDARY", function () {
  it("Should be an ERC20 token", async function () {
    const [owner] = await ethers.getSigners();
    const BMHTL = await ethers.getContractFactory("BMHTL");
    const token = await BMHTL.deploy(owner.address);
    await token.deployed();
    expect(await token.name()).to.equal("Mouse Haunt Booster LEGENDARY");
    expect(await token.symbol()).to.equal("BMHTL");
    expect(await token.totalSupply()).to.equal(ethers.utils.parseEther("8250"));
  });
  it("Should be pausable", async function () {
    ...
    await expect(
      token.transferFrom(owner.address, to.address, 1)
  ...
  it("Should setup allowance to marketing account", async function () {
    ...
    const tx = token
      .connect(marketing)
      .transferFrom(owner.address, to.address, 42);

    await expect(tx)
      .to.emit(token, "Transfer")
      .withArgs(owner.address, to.address, 42);
  });
});
