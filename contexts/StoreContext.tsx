...
export const StoreContext = createContext<StoreContextData>({} as StoreContextData)
...
function checkHasBoughtWhitelistButNotClaimed(userInfo: {
  ...
  return !userInfo?.remainingTokens.isZero() && userInfo.remainingTokens.eq(userInfo.totalTokens)
}
...
async function getUserInfo(contracts: Contracts, account: string): Promise<UserInfo> {
  ...
  const amounts = contracts.whitelistSales
    .map((sale, index) => {
      ...
      const { unlockAtIGOPercent } = details
      const { vestingPeriodMonths } = details
      const igoAmount =
        unlockAtIGOPercent && userInfo && userInfo.totalTokens
          ? userInfo.totalTokens.mul(unlockAtIGOPercent).div(100)
          : ethers.utils.parseEther('0')
      const monthlyAmount =
        unlockAtIGOPercent && userInfo && userInfo.totalTokens && vestingPeriodMonths
  ...
  const igoAmount = amounts ? ethers.utils.formatEther(amounts.igoAmount.toString()) : ''
  const monthlyAmount = amounts ? ethers.utils.formatEther(amounts.monthlyAmount.toString()) : ''
  const claimsPerMonth = amounts ? amounts.claimsPerMonth : 0

  const totalTokens = userInfos.map((userInfo) => userInfo.totalTokens).reduce((a, b) => a.add(b))
  ...
  const lastClaimMonthIndex = userInfos
    .map((userInfo) =>
      !isEmptyUserInfo(userInfo) ? userInfo.lastClaimMonthIndex.toString() : '-1'
    )
    .sort()
    .slice(-1)
    .pop()
  ...
  return {
    ...
    lastClaimMonthIndex: typeof lastClaimMonthIndex === 'string' ? Number(lastClaimMonthIndex) : -1,
    igoAmount,
    monthlyAmount,
    claimsPerMonth,
...
export const StoreProvider: React.FC<Props> = ({ children }: Props) => {
  ...
  const [userInfoDetailed, setUserInfoDetailed] = useState<UserInfoDetailed | undefined>()
      ...
      ;(async () => {
        try {
          ...
          const whitelistSales = sales
            ...
            .map(
              (address) =>
                new ethers.Contract(address, WhitelistSaleJson.abi, signer) as WhitelistSale
            )
          ...
          for await (const sale of whitelistSales) {
            ...
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            const isPreSaleWhitelisted = details && (details as any).whitelisted === account
    ...
    if (account && contracts) {
      ;(async () => {
        try {
          ...
          const mhtOnWallet = ethers.utils.formatEther(await contracts.mht.balanceOf(account))
          ...
          const { hasBoughtWhitelistButNotClaimed } = userInfo
